...
import { Plus, Trash2, ArrowRight, Check, Loader, HelpCircle, Wand2, Edit2, X } from 'lucide-react';
...
function QuestionForm({ initialText = '', initialAnswers = ['', '', '', ''], initialCorrect = 1, onSave, onCancel, saving, label }) {
  ...
  const handleSave = () => {
    if (!questionText.trim()) { alert('נא להזין את תוכן השאלה'); return; }
    if (answers.some(a => !a.trim())) { alert('נא למלא את כל ארבע התשובות'); return; }
    onSave({ questionText: questionText.trim(), answers: answers.map(a => a.trim()), correctIndex });
  };
    ...
    <div className="space-y-4">
        ...
        {answers.map((answer, idx) => (
          <div key={idx} className="flex items-center gap-2">
            <button
              ...
              className={`h-10 w-10 rounded-xl flex items-center justify-center transition shrink-0 border text-sm font-bold ${correctIndex === idx + 1 ? 'bg-emerald-500 border-emerald-400 text-white' : ''}`}
      ...
      <div className="flex gap-2">
        <button
          onClick={handleSave}
          disabled={saving}
          className="flex-1 text-white font-bold py-3 rounded-xl transition flex items-center justify-center gap-2 disabled:opacity-50 hover:scale-[1.01]"
...
function QuestionsManager({ topic, onClose, onShowAI }) {
  ...
  const loadQuestions = async () => {
    ...
    const { data, error } = await supabase
      .from('questions').select('*').eq('topic_id', topic.id).order('created_at', { ascending: true });
    if (!error && data) setQuestions(data);
    setLoading(false);
  };

  const addQuestion = async ({ questionText, answers, correctIndex }) => {
    setSaving(true);
    const { error } = await supabase.from('questions').insert({
      topic_id: topic.id,
      question_text: questionText,
      answer_1: answers[0], answer_2: answers[1], answer_3: answers[2], answer_4: answers[3],
      correct_index: correctIndex,
    });
    if (error) alert('שגיאה בשמירת השאלה: ' + error.message);
    else await loadQuestions();
    setSaving(false);
  };

  const updateQuestion = async (id, { questionText, answers, correctIndex }) => {
    setSaving(true);
    ...
    if (error) alert('שגיאה בעדכון השאלה: ' + error.message);
    else { setEditingId(null); await loadQuestions(); }
    setSaving(false);
  };
  ...
  return (
    <div className="min-h-screen p-4 md:p-8 max-w-4xl mx-auto" dir="rtl" style={{ background: C.bg }}>
      ...
      <div className="p-5 mb-6 rounded-2xl"
        ...
        <h2 className="text-sm font-bold mb-4" style={{ color: C.pink }}>הוספת שאלה חדשה</h2>
        <QuestionForm onSave={addQuestion} saving={saving} label="הוספת השאלה" />
      </div>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {questions.map((q, idx) => {
              ...
              return (
                <div key={q.id} className="p-4 rounded-2xl" style={{ background: 'rgba(255,255,255,0.85)', border: `1px solid ${isEditing ? C.pink : 'rgba(239,144,152,0.15)'}` }}>
                  <div className="flex justify-between items-start mb-3">
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      ...
                      {!isEditing && <span className="font-bold text-sm" style={{ color: C.dark }}>{q.question_text}</span>}
                    </div>
                  ...
                  {isEditing ? (
                    <QuestionForm
                      ...
                      initialAnswers={ans}
                      initialCorrect={q.correct_index}
                      onSave={(vals) => updateQuestion(q.id, vals)}
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {ans.map((a, i) => (
                        <div key={i}
                          className={`text-sm px-3 py-2 rounded-lg border flex items-center gap-2 ${q.correct_index === i + 1 ? 'bg-emerald-500/10 border-emerald-500/30 text-emerald-700 font-bold' : ''}`}
